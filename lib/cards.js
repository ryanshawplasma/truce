...
import { bucketByMonth, countBy, median, openDelays } from './analytics';
 ...
 *
 *  Fetched newest-first and then flipped: a card that hits the 50-reaction cap
 *  should show the 50 most RECENT reactions, not the 50 oldest — otherwise the
 *  most enthusiastic recipients see a frozen list from their first minute. */
...
/**
 * The most rows the twelve-month trend will read.
 *
 * It selects four timestamp-ish columns and nothing anybody wrote, so the
 * rows are small — but "small times unbounded" is still unbounded, and a
 * stats page is not worth an out-of-memory on the day the site works. Past
 * this the page says the chart is partial rather than quietly under-counting
 * the most recent month, which is the one anybody is looking at.
 */
...
export async function getAdminStats() {
  ...
  /* How the three occasions are actually being used. One cheap count each —
     still ids only, so this cannot leak anything anybody wrote. */
  const occasionCounts = {};
  await Promise.all(
    OCCASION_IDS.map(async (id) => {
      occasionCounts[id] = await countRows(() =>
        supabase.from('cards').select('id', { count: 'exact', head: true }).eq('occasion', id),
      );
    }),
  );

  /* One read for the whole trend: twelve months of created_at / opened_at,
     plus the two columns the breakdowns need. Twelve separate COUNT queries
     would be twelve round trips to draw one chart. */
  const yearAgo = new Date(Date.now() - 370 * 24 * 60 * 60 * 1000).toISOString();
  const { data: trendRows, error: trendError } = await supabase
    .from('cards')
    .select('created_at, opened_at, occasion, theme')
    .gte('created_at', yearAgo)
    .order('created_at', { ascending: false })
    .limit(TREND_ROW_LIMIT);

  if (trendError) console.error('[truce] trend failed:', trendError.message);

  const rows = trendRows || [];
  const monthly = bucketByMonth(rows, { months: 12 });
  const themeCounts = countBy(rows, 'theme');
  const medianOpenMs = median(openDelays(rows));

  /* "Our corner" lives in its own tables and may not exist at all on a
     database that has only ever made cards. Both counts fail to null and
     the page renders a dash rather than a zero it cannot stand behind. */
  const [rooms, roomMessages] = await Promise.all([
    countRows(() => supabase.from('couple_rooms').select('id', { count: 'exact', head: true })),
    countRows(() => supabase.from('couple_messages').select('id', { count: 'exact', head: true })),
  ]);
  ...
  return {
    ...
    rooms,
    roomMessages,
    /* The chart is drawn from a capped read; say so rather than let the
       oldest months look like a decline that never happened. */
    trendCapped: rows.length >= TREND_ROW_LIMIT,
    recent: recent || [],
