...
/**
 * What signs the corner's session cookies — the fact, never the secret.
 *
 * WHY THIS IS WORTH REPORTING
 * ---------------------------
 * When ADMIN_SECRET is unset the signing key is DERIVED from the Supabase
 * service-role key. That works, and it has one sharp edge: rotating the
 * database key silently invalidates every session cookie in existence. The
 * symptom is a corner that is created successfully and then refuses to open,
 * reporting a sign-in it cannot verify — which looks like a browser problem
 * and is not one.
 *
 * That is a hard thing to work out from the outside and a trivial thing to
 * read off the server, so /dev reads it off the server.
 *
 * Returns booleans and a label. It never returns, logs or hashes anything
 * derived from either secret: knowing THAT a key is set is a configuration
 * fact; knowing anything about its value is a leak.
 */
export function sessionKeyInfo() {
  const explicit = Boolean(process.env.ADMIN_SECRET);
  const derivable = Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_URL);

  if (explicit) {
    return {
      source: 'ADMIN_SECRET',
      stable: true,
      note: 'Sessions survive a database key rotation.',
    };
  }
  if (derivable) {
    return {
      source: 'derived from the Supabase key',
      stable: false,
      note:
        'ADMIN_SECRET is not set, so sessions are signed with a key derived from ' +
        'SUPABASE_SERVICE_ROLE_KEY. Rotating that key signs everybody out and makes ' +
        'corners refuse to open until they sign in again. Set ADMIN_SECRET to a long ' +
        'random string to make sessions independent of it.',
    };
  }
  return {
    source: 'development fallback',
    stable: false,
    note: 'Neither ADMIN_SECRET nor a Supabase key is set, so corners are not really protected.',
  };
}
