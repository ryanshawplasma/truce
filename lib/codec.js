...
import { DEFAULT_OCCASION, safeOccasion } from './occasions';
...
export function toPayload(card) {
  ...
  /* Older links have no `oc` at all, so the default occasion is the one that
     costs nothing to leave out — every apology link ever made still decodes. */
  if (card.occasion && card.occasion !== DEFAULT_OCCASION) d.oc = card.occasion;
