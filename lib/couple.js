...
export async function markTyping(roomId, side) {
  ...
  if (error && logPgError('markTyping', error) === 'schema') notePresenceMissing('markTyping');
}
...
export async function readPresence(roomId, side) {
  ...
  const { data, error } = await supabase
    .from('couple_rooms')
    .select(`seen_at_${theirs}, read_upto_${theirs}, typing_at_${theirs}, receipts_${theirs}, receipts_${side === 2 ? '2' : '1'}`)
    .eq('id', roomId)
    .maybeSingle();

  if (error) {
    if (logPgError('readPresence', error) === 'schema') notePresenceMissing('readPresence');
    return null;
  }
  if (!data) return null;
