 ...
 */
/**
 * How long a "this column does not exist" verdict stands before we look again.
 *
 * WHY IT HAS TO EXPIRE
 * --------------------
 * These flags used to be one-way: once a process decided a column was missing,
 * it never asked again. Which is fine right up to the moment somebody runs the
 * migration — because every warm serverless instance that had already served a
 * corner keeps the feature switched off, in the same deployment, against a
 * database that now has the column. The symptom is the worst kind: you paste
 * the SQL, it succeeds, and reactions still do not work, with nothing in the
 * logs because from that process’s point of view nothing changed.
 *
 * So a verdict is now a cache with a five-minute life. When it lapses the next
 * query asks for the full column list again and finds out. The cost of being
 * wrong is one failed query per five minutes per process, which is nothing; the
 * cost of never asking is a feature that stays dead until a redeploy nobody
 * knows they need.
 */
const RECHECK_MISSING_MS = 5 * 60 * 1000;

let mediaMissingAt = 0;
let extrasMissingAt = 0;
let durationMissingAt = 0;
let editMissingAt = 0;

/**
 * Let stale verdicts lapse back to "unknown".
 *
 * Called at the top of everything that touches a message, so the dozens of
 * `=== 'missing'` reads further down need no changes: by the time they run, the
 * flag is already current.
 */
export function expireMissingFlags(now = Date.now()) {
  if (mediaColumn === 'missing' && now - mediaMissingAt >= RECHECK_MISSING_MS) mediaColumn = 'unknown';
  if (extrasColumns === 'missing' && now - extrasMissingAt >= RECHECK_MISSING_MS) extrasColumns = 'unknown';
  if (durationColumn === 'missing' && now - durationMissingAt >= RECHECK_MISSING_MS) durationColumn = 'unknown';
  if (editColumn === 'missing' && now - editMissingAt >= RECHECK_MISSING_MS) editColumn = 'unknown';
}
...
/**
 * seen_at_N / read_upto_N on couple_rooms.
 *
 * The room used to draw one tick, because nothing recorded whether anybody
 * had read anything and a second tick would have been decoration pretending
 * to be information. These four columns are what make it honest.
 *
 * Same contract as every other optional column here: the first 42703 that
 * names one of them switches presence off, logs the SQL, and the room carries
 * on with a single tick and no "last seen". Losing a tick is a worse day;
 * losing the conversation is a broken site.
 */
let presenceColumns = 'unknown'; // 'unknown' | 'present' | 'missing'
let presenceMissingAt = 0;

/**
 * Section 7 — typing_at_N and receipts_N — tracked SEPARATELY from section 6.
 *
 * They arrived two sections apart and readPresence asked for all of them in
 * one select, so a database with section 6 and not section 7 answered 42703
 * and switched the whole lot off: no double ticks, no last seen, no typing,
 * on a deployment whose tick columns were sitting right there and working.
 * That is precisely the failure this file's degradation was written to
 * prevent, reintroduced by putting two generations of column in one query.
 *
 * One flag per section means a half-upgraded database loses exactly the half
 * it is missing.
 */
let typingColumns = 'unknown'; // 'unknown' | 'present' | 'missing'
let typingMissingAt = 0;

export function isPresenceMissing() {
  return presenceColumns === 'missing';
}

export function isTypingMissing() {
  return typingColumns === 'missing';
}

/**
 * Let a stale 'missing' verdict lapse back to 'unknown'.
 *
 * Shared by both presence calls on purpose. It lived inside markPresence,
 * which happens to run first on every poll — so readPresence recovered by
 * ordering rather than by design, and would have been stuck forever the
 * moment anything called it on its own. That is exactly the trap the
 * expireMissingFlags work was written to close, reintroduced two columns
 * later.
 */
function expirePresenceVerdict(now = Date.now()) {
  if (presenceColumns === 'missing' && now - presenceMissingAt >= RECHECK_MISSING_MS) {
    presenceColumns = 'unknown';
  }
  if (typingColumns === 'missing' && now - typingMissingAt >= RECHECK_MISSING_MS) {
    typingColumns = 'unknown';
  }
}

function notePresenceMissing(scope) {
  if (presenceColumns === 'missing') return;
  presenceColumns = 'missing';
  presenceMissingAt = Date.now();
  console.error(
    `[truce] ${scope}: couple_rooms has no seen_at_1/2 or read_upto_1/2, so ticks and ` +
      'last seen are switched off. Run supabase/upgrade.sql, section 6.',
  );
}

function noteTypingMissing(scope) {
  if (typingColumns === 'missing') return;
  typingColumns = 'missing';
  typingMissingAt = Date.now();
  console.error(
    `[truce] ${scope}: couple_rooms has no typing_at_1/2 or receipts_1/2, so "typing…" and ` +
      'the read-receipt switch are off. Ticks and last seen are unaffected. ' +
      'Run supabase/upgrade.sql, section 7.',
  );
}

/**
 * Route one 42703 to the section it actually names.
 *
 * The old code sent every schema error to notePresenceMissing, which printed
 * "couple_rooms has no seen_at_1/2 … run section 6" while the error in hand
 * said `column couple_rooms.typing_at_1 does not exist`. A log line that names
 * the wrong cure sends whoever reads it to re-run SQL they have already run.
 */
function notePresenceSchema(scope, error) {
  const message = String((error && error.message) || '');
  if (/typing_at_|receipts_/.test(message)) return noteTypingMissing(scope);
  if (/seen_at_|read_upto_/.test(message)) return notePresenceMissing(scope);
  /* Unnamed: assume the newer section, which is the likelier gap and the
     cheaper thing to lose. */
  return typingColumns === 'missing' ? notePresenceMissing(scope) : noteTypingMissing(scope);
}

/**
 * Say "I am here", and how far I have read.
 *
 * Called from the room’s own four-second poll, so simply having the room open
 * keeps presence current. `readUpto` only moves forward: a reader scrolled up
 * into last week must not un-read what they have already seen.
 */
export async function markPresence(roomId, side, readUpto) {
  const supabase = getSupabase();
  if (!supabase) return;

  expirePresenceVerdict();
  if (presenceColumns === 'missing') return;

  const seat = side === 2 ? '2' : '1';
  const patch = { [`seen_at_${seat}`]: new Date().toISOString() };

  const upto = Number(readUpto);
  if (Number.isFinite(upto) && upto > 0) patch[`read_upto_${seat}`] = Math.round(upto);

  const { error } = await supabase
    .from('couple_rooms')
    .update(patch)
    .eq('id', roomId)
    /* Never walk the mark backwards. Two tabs, one scrolled up, would
       otherwise fight and the other side would watch blue ticks flicker. */
    .or(`read_upto_${seat}.is.null,read_upto_${seat}.lte.${Math.round(upto) || 0}`);

  if (error) {
    if (logPgError('markPresence', error) === 'schema') notePresenceMissing('markPresence');
    return;
  }
  if (presenceColumns === 'unknown') presenceColumns = 'present';
}

/**
 * What the OTHER side has been up to: { seenAt, readUpto }.
 *
 * Null when the columns are not there, which the UI reads as "no information"
 * and falls back to a single tick rather than inventing one.
 */
/**
 * "I am typing."
 *
 * Its own call rather than a field on the poll, because the poll is every
 * four seconds and typing that shows up four seconds late has missed the
 * point. The client rate-limits it to one ping every few seconds, so this
 * is cheaper than it looks.
 */
export async function markTyping(roomId, side) {
  const supabase = getSupabase();
  if (!supabase) return;

  expirePresenceVerdict();
  /* Only its own columns gate this one. A room with no typing columns still
     has ticks worth keeping. */
  if (typingColumns === 'missing') return;

  const seat = side === 2 ? '2' : '1';
  const { error } = await supabase
    .from('couple_rooms')
    .update({ [`typing_at_${seat}`]: new Date().toISOString() })
    .eq('id', roomId);

  if (error && logPgError('markTyping', error) === 'schema') notePresenceSchema('markTyping', error);
}

export async function readPresence(roomId, side) {
  const supabase = getSupabase();
  if (!supabase) return null;

  expirePresenceVerdict();
  if (presenceColumns === 'missing') return null;

  const theirs = side === 2 ? '1' : '2';
  const mine = side === 2 ? '2' : '1';

  /* Ask for section 7 only while it might be there. */
  const columnsFor = (withTyping) =>
    [`seen_at_${theirs}`, `read_upto_${theirs}`]
      .concat(withTyping ? [`typing_at_${theirs}`, `receipts_${theirs}`, `receipts_${mine}`] : [])
      .join(', ');

  let wantedTyping = typingColumns !== 'missing';
  let { data, error } = await supabase
    .from('couple_rooms')
    .select(columnsFor(wantedTyping))
    .eq('id', roomId)
    .maybeSingle();

  if (error) {
    if (logPgError('readPresence', error) !== 'schema') return null;
    notePresenceSchema('readPresence', error);

    /* Ask again without the columns that just failed, so the ticks this
       deployment CAN serve arrive on this poll instead of the next one. A
       first-ever poll used to return nothing at all here, which read as
       "never seen" for four seconds and made the header flicker. */
    if (!wantedTyping || typingColumns !== 'missing') return null;
    wantedTyping = false;
    ({ data, error } = await supabase
      .from('couple_rooms')
      .select(columnsFor(false))
      .eq('id', roomId)
      .maybeSingle());
    if (error) {
      if (logPgError('readPresence(retry)', error) === 'schema') {
        notePresenceMissing('readPresence(retry)');
      }
      return null;
    }
  }
  ...
  if (presenceColumns === 'unknown') presenceColumns = 'present';
  if (wantedTyping && typingColumns === 'unknown') typingColumns = 'present';

  /* Reciprocal, the way WhatsApp does it: switching your own receipts off
     also stops you seeing theirs. Anything else lets somebody read in
     silence while still watching the other person be read, which is the
     one arrangement nobody would agree to out loud.

     Only the read mark is withheld. "Delivered" and "last seen" are not
     receipts — they say a browser fetched something, not that a person
     looked at it. */
  /* `!== false` rather than a truthiness check, which is what makes the four
     lines below correct on a database without section 7: an absent column
     reads as undefined, undefined is not false, and the room falls back to
     receipts on and nobody typing — exactly what it did before the switch
     existed. */
  const receiptsOn = data[`receipts_${mine}`] !== false && data[`receipts_${theirs}`] !== false;

  return {
    seenAt: data[`seen_at_${theirs}`] || null,
    readUpto: receiptsOn ? Number(data[`read_upto_${theirs}`]) || 0 : 0,
    typingAt: data[`typing_at_${theirs}`] || null,
    receipts: data[`receipts_${mine}`] !== false,
  };
}
