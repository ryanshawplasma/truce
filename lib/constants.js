...
/* Who the card is for. `tag` is what we filter the message library by.
   This is the registry of every recipient we know about; each occasion picks
   the subset it offers — see occasionRecipients() in lib/occasions.js. */
export const RECIPIENTS = [
  { id: 'girlfriend', label: 'Girlfriend', emoji: '💕', tag: 'romantic' },
  { id: 'boyfriend', label: 'Boyfriend', emoji: '💙', tag: 'romantic' },
  /* Proposals only. Deliberately not offered on the everyday occasions, where
     eight cards already cover the ground and a ninth just adds a decision. */
  { id: 'partner', label: 'Partner', emoji: '💞', tag: 'romantic' },
  { id: 'wife', label: 'Wife', emoji: '💍', tag: 'romantic' },
  { id: 'husband', label: 'Husband', emoji: '🤵', tag: 'romantic' },
  /* Siblings. Absent until Raksha Bandhan needed them, which is its own
     small indictment: a card app with a Mom and a Dad and no brother or
     sister. Tagged 'family' so the existing message filtering picks them up
     with no special cases. */
  { id: 'sister', label: 'Sister', emoji: '🌸', tag: 'family' },
  { id: 'brother', label: 'Brother', emoji: '🧿', tag: 'family' },
...
export const THEMES = [
  ...
  /* Seasonal. Offered only around Raksha Bandhan — see lib/festival.js — but
     it lives here permanently and always renders, because a card made on the
     day has to keep its look forever. `seasonal` is what the picker reads. */
  {
    id: 'rakhi',
    label: 'Rakhi Gold',
    seasonal: 'rakhi',
    dots: ['#FFE7BC', '#D4451F', '#FFF9EF'],
    bg: 'linear-gradient(160deg,#FFF4DF,#FFD79B)',
  },
];
...
/**
 * The sample card, with a timestamp.
 *
 * SAMPLE_CARD has no created_at, which is fine for a fixture and wrong for a
 * demonstration: the keepsake image drew no date under the signature and the
 * line about how long the letter waited never appeared at all. Two real
 * features were invisible on /c/demo — the one card most people ever open.
 *
 * Worked out per request rather than baked in, or the demo would slowly begin
 * claiming to have been written years ago.
 */
export function sampleCard(now = Date.now()) {
  return { ...SAMPLE_CARD, created_at: new Date(now - 2 * 24 * 60 * 60 * 1000).toISOString() };
}
